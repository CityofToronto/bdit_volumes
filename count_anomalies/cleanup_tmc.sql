...
WHERE count_info_id IN
	...
	FROM prj_volume.det_clean JOIN traffic.countinfomics USING (count_info_id)
...
WHERE count_info_id IN (SELECT count_info_id
	FROM (SELECT count_info_id, timecount
		...
		HAVING COUNT(*) > 1) B
	GROUP BY count_info_id
	HAVING count(*) = (SELECT COUNT(DISTINCT count_time::time) FROM prj_volume.det_clean C WHERE B.count_info_id = C.count_info_id)));
