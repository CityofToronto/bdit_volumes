...
SELECT g1, dir_bin, place_holder_time_var, array_agg(v ORDER BY parallel, row_number)
FROM(
	SELECT g1, C.dir_bin, g2, place_holder_time_var, parallel, dist, row_number() OVER (PARTITION BY g1, parallel ORDER BY dist, feature_code), E.volume::int as v
	FROM 
		...
		FROM (SELECT t1.group_number AS g1, t2.group_number AS g2, ST_Distance(t1.shape, t2.shape) AS dist, t2.feature_code, 
			...
			WHERE t1.feature_code=$1 AND t1.group_number != t2.group_number AND t2.group_number IN (SELECT DISTINCT group_number FROM prj_volume.place_holder_table_name) AND t1.group_number NOT IN (SELECT DISTINCT group_number FROM prj_volume.place_holder_table_name)) A
