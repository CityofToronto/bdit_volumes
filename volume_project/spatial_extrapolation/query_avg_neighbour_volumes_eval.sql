...
WITH segments AS (
	SELECT group_number, place_holder_time_var, AVG(volume) AS volume, shape, feature_code
...
SELECT g1, place_holder_time_var, AVG(neighbourvolume)::int, volume::int
FROM (SELECT l1.group_number AS g1, l1.volume as volume, l2.volume as neighbourvolume, place_holder_time_var, row_number() OVER (PARTITION BY l1.group_number,l1.volume ORDER BY ST_Distance(l1.shape, l2.shape))
