...
BEGIN
	...
	remaining := TRUE;	-- checks whether any centreline_ids remain without a group
	...
	WHILE remaining = TRUE LOOP
		...
		WHILE group_loop = TRUE LOOP
			grouped_cls_new := array(SELECT DISTINCT cl2 FROM prj_volume.centreline_volumes_truth WHERE dir_bin = current_db AND unused = TRUE and same_volume = TRUE AND cl1 = ANY(grouped_cls) AND NOT cl2 = ANY(grouped_cls));

			IF (array_length(grouped_cls_new,1) IS NULL) THEN
				group_loop = FALSE;
