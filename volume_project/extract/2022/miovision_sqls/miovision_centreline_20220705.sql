...
CREATE TABLE scannon.centreline_miovision_20220705 AS (
    ...
    WITH cent_int AS (
        ...
        LEFT JOIN gis.centreline_20220705 AS cent ON ter.int_id = cent.tnode
        LEFT JOIN gis.centreline_intersection_20220705 AS ci ON cent.fnode = ci.int_id
        ...
        SELECT DISTINCT
            ...
            ci.longitude
        FROM miovision_api.intersections AS ter
        LEFT JOIN gis.centreline_20220705 AS cent ON ter.int_id = cent.fnode
        LEFT JOIN gis.centreline_intersection_20220705 AS ci ON cent.tnode = ci.int_id
        WHERE cent.fcode_desc NOT IN (
            'Trail',
            'Geostatistical line',
            'Other',
...
--3. There were some cases where there was data for an intersection+leg combo that wasn't in my look up table, so I did some investigations and manually updated accordingly.
--of the missing intersection+leg combos, two were for non-streets, so there's nothing to map them to in the centreline.
