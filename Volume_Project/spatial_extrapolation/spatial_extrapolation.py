...
class spatial_extrapolation(vol_utils):    
    ...
    def Linear_Regression_Prox(self, data, road_class):
        ...
        for i in range(10):
            ...
            if i == 9:
                ...
                ax.set_ylim(x)
                ax.plot(x,x)
                ax.set_title(self.rc_lookup[road_class] + ' Linear Regression (by proximity) \n with ' + str(i+2) + ' neighbours', fontsize = 13)
                ax.set_xlabel('Predicted Volume (veh)')
                ax.set_ylabel('Observed Volume (veh)')

                ax.annotate('Root Mean Squared Error: ' + "{:.0f}".format(np.sqrt(mean_squared_error(y_test,y_predict))), xy=((x[1]-x[0])*0.04+x[0], x[1]*0.92), fontsize = 11)
                ax.annotate('Coef of Det: ' + "{:.3f}".format(regr.score(x_test, y_test)), xy=((x[1]-x[0])*0.04+x[0], x[1]*0.86), fontsize = 11)
                
                fig.savefig('spatial_extrapolation/img/'+self.rc_lookup[road_class].lower().replace(' ', '_')+'_proximity_regr.png')
    ...
    def Linear_Regression_Directional(self, road_class):
        ...
        ax.scatter(y_predict, y_test)
        ax.set_title(self.rc_lookup[road_class] + ' Directional Linear Regression \n with 2 parallel and 2 perpendicular')
        ax.set_xlabel('Predicted Volume (veh)')
        ax.set_ylabel('Observed Volume (veh)')
        ...
        ax.annotate('Coef of Det: ' + "{:.3f}".format(regr.score(x_test, y_test)), xy=((x[1]-x[0])*0.06+x[0], x[1]*0.86), fontsize = 11)
        fig.savefig('spatial_extrapolation/img/'+self.rc_lookup[road_class].lower().replace(' ','_') +'_directional_regr.png')
        
    def Average_Neighbours(self, data):
        dist = np.array(data[['from_x','from_y','to_x','to_y']])
        kdt = KDTree(dist, 12)
